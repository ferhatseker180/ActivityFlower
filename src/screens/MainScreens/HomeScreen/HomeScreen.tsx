...
const HomeScreen = ({navigation}: any) => {
  ...
  const renderActivity = ({item}: any) => (
    <MainCard
      activity={item}
      function={() => navigation.navigate('HomeDetailScreen', {home: item})}
    />
  );
  ...
  const handleSearch = (text: any) => {
    const filteredList = main_data.filter(data => {
      ...
      return (
        currentName.includes(text) ||
        currentCategory.includes(text) ||
        currentLocation.includes(text)
      );
    });
  ...
  const handleDateRangeSelection = (startDate: any, endDate: any) => {
    const filteredList = main_data.filter(activity => {
      ...
      return (
        ...
        activityDate.isSameOrBefore(endDate)
      );
    });
      ...
      <FlatList
        ListHeaderComponent={
          <>
            ...
            <RatingScrollView navigation={navigation} />
          </>
        }
